-- ============================================================
--  SPORT PERFORMANCE PLATFORM v2.0 — Supabase Migration
--  Colar no SQL Editor: https://app.supabase.com → SQL Editor
-- ============================================================
...
create table if not exists clubs (
  ...
  name text not null, season text, color text, logo text,
...
create table if not exists athletes (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  team_id text, name text not null, pos text, num int, dob text,
  nat text, height text, foot text, photo text, club text,
  created_at timestamptz default now()
);
...
create table if not exists wellness_records (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  athlete_id text, date text, sleep int, fatigue int, stress int,
  pain int, mood int, score int, cycle text,
...
create table if not exists pse_records (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  athlete_id text, date text, pse numeric, duration int, ua numeric,
  session_type text, submitted_at timestamptz default now()
);

-- 6. LESÕES
create table if not exists injuries (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  athlete_id text, date text, type text, body_part text, diagnosis text,
  severity text, status text default 'ativo', return_date text,
  phase int default 1, phase_desc text, progress int default 0, notes text,
...
-- 8. AVALIAÇÕES NUTRIÇÃO
create table if not exists nutrition_records (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  athlete_id text, date text, weight numeric, height numeric, bmi numeric,
  body_fat numeric, muscle numeric, tricipital numeric, suprailiaca numeric,
  hydration text, notes text, registered_at timestamptz default now()
);

-- 9. PLANOS NUTRICIONAIS
...
create table if not exists schedule_events (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  team_id text, type text, title text, date text, time text,
  location text, dur int, notes text,
  created_at timestamptz default now()
);

-- 11. TREINOS INDIVIDUALIZADOS
create table if not exists training_plans (
  id text primary key, club_id uuid references clubs(id) on delete cascade,
  athlete_id text, title text, type text, description text, note text,
  week text, volume text, done boolean default false, done_at text,
  created_at timestamptz default now()
);
...
-- Policies (acesso via anon key — protegido por club_id UUID)
